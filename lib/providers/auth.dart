...
class AuthProvider with ChangeNotifier {
  ...
  Future<bool> RegisterPin(
      ...
      String deviceId,
      String pin}) async {
    final __res = await HttpService(context, Api.API_REGISTER_PIN).body({
      'type': 'DEVICE_PIN',
      'email': email,
      'deviceId': deviceId,
      'pin': pin
  ...
  Future<bool> signupByEmail(
    ...
    final __res = await HttpService(context, Api.API_USER_SIGNUP).body({
      "nickname": nickname,
      "password": password,
      "consents": consents,
      "language": "en"
    ...
    final __res1 = await HttpService(context, Api.API_SIGNUP_STEP_CREATE + "EMAIL")
  ...
  Future<bool> resendEmailOrPhone(
      {@required BuildContext context,
      String type,
      String value,
      String nickname}) async {
    String tp = "EMAIL";
    if (type == "phone") tp = "PHONE";
    final __res = await HttpService(context, Api.API_SIGNUP_STEP_CREATE + tp)
  ...
  Future<bool> ConfirmEmailOrPhone(
      {@required BuildContext context,
      String type,
      String value,
      String pin,
    ...
    final __res = await HttpService(context, Api.API_SIGNUP_STEP_CONFIRM + tp)
  ...
  void _autoLogout() {
    _cancelAuthTimer();
    print(
        "logout timer: ${(JwtDecoder.getExpirationDate(token).difference(DateTime.now())).inSeconds}");
    _authTimer = Timer(
        Duration(
            seconds:
